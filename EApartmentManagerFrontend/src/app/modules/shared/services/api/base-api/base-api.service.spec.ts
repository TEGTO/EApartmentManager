...
describe('BaseApiService', () => {
  ...
  it('should combine path with API URL', () => {
    ...
    mockUrlDefiner.combineWithApartmentApiUrl.and.returnValue('http://api.example.com/test-path');
    const combinedUrl = service['combinePathWithApartmentApiUrl'](path);
    expect(combinedUrl).toBe('http://api.example.com/test-path');
    expect(mockUrlDefiner.combineWithApartmentApiUrl).toHaveBeenCalledWith(path);
  });
